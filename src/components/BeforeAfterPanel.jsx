...
function FieldRow({ label, raw, clean, improved }) {
  return (
    <div className="grid grid-cols-[60px_1fr_20px_1fr] gap-2 items-center py-1.5 border-b border-slate-100 last:border-0">
      ...
      <div className="flex items-center gap-1">
        {clean ? (
          <span className={`text-base font-mono px-2 py-0.5 rounded border truncate max-w-[140px] ${
      ...
      <AnimatePresence>
        {samples.map((s, i) => (
          ...
          >
            ...
            />
            {i < sampleSize - 1 && <div className="border-b-2 border-dashed border-slate-100 my-1" />}
          </motion.div>
        ))}
      </AnimatePresence>
